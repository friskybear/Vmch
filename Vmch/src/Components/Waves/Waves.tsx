...
export function Waves({
  ...
  useEffect(() => {
    ...
    function movePoints(time: number) {
      ...
      lines.forEach((pts) => {
        pts.forEach((p: { x: number; y: number; wave: { x: number; y: number; }; cursor: { vx: number; vy: number; x: number; y: number; }; }) => {
          const move =
            noise.perlin2(
    ...
    function drawLines() {
      ...
      if (ctx) {
        ...
        linesRef.current.forEach((points) => {
          ...
          points.forEach((p: { x: number; y: number; wave: { x: number; y: number; }; cursor: { x: number; y: number; vx: number; vy: number; }; }, idx: number) => {
